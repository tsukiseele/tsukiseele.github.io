...
export const useMainStore = defineStore('main', {
  ...
  actions: {
    ...
    async getArchives({ page, count }) {
      ...
      const result = await app.$api.getArchives({ page, count })
      if (result) {
    ...
    async getImages() {
      ...
      const app = useNuxtApp()
      this.images = [].concat(...(await app.$api.getArchives({ page: 1, count: 99 })).map((item) => formatGallery(item))).slice(0, 20)
    },
    /**
     * 获取时间线
     * @param {*} param0
    ...
    async getLabels() {
      ...
      // 如果没有找到就请求
      const app = useNuxtApp()
      this.labels = await app.$api.getLabels()
    },
    /**
     * 获取分类列表
     * @param {Context} context 上下文
    ...
    async getCategorys() {
      ...
      const app = useNuxtApp()
      this.categorys = await app.$api.getMilestones()
    },
    /**
     *
     * @param {*} param0
    ...
    async getInspiration({ page, count }) {
      const app = useNuxtApp()
      this.inspiration = formatInspiration(await app.$api.getInspiration({ page, count }))
    },
    /**
     * 获取关于
     * @param {*} param0
    ...
    async getAbout() {
      ...
      const app = useNuxtApp()
      const about = await app.$api.getPage('about')
      if (about && about[0]) {
    ...
    async getProjects() {
      ...
      const app = useNuxtApp()
      const projects = (await Promise.all([app.$api.getPage('projects'), app.$api.getPage('websites')])).map(item => item[0])
    ...
    async getExperience() {
      ...
      const app = useNuxtApp()
      const experience = await app.$api.getPage('experience')
